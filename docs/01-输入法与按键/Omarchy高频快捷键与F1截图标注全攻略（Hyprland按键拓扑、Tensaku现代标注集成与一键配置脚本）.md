...
| **保存流程** | 需在弹窗中选择路径保存 | **按回车 (`Enter`) 瞬间复制到剪贴板，后台自动保存存档** |
...
### 3. 保存与分享阶段
* **极速回车复制**：标注完成后，直接敲击键盘 **`Enter` (回车)** 或点击绿色的对勾复制图标：
  1. 标注后的高清图像立即存入系统剪贴板（`wl-copy`），你可以直接在微信、飞书、网页输入框或代码编辑器中按 `Ctrl + V` 粘贴发送；
  2. 图像同时以带时间戳的文件名（如 `tensaku-2026-09-15_00-30-00.png`）自动保存在 `~/Pictures/` 目录下备查。
